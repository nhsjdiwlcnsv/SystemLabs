//
// Created by Mikhail Shkarubski on 5.11.23.
//
#pragma once
...
HANDLE hRefreshEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
...
DWORD WINAPI UpdateThreadProc(LPVOID lpParam) {
    ...
    while (true) {
        WaitForSingleObject(hRefreshEvent, INFINITE);
        ...
        for (DWORD i = 0; i < cProcesses; i++) {
            ...
            wchar_t memoryUsageDisplay[256] = { 0 };
            getMemoryInfo(hProcess, memoryUsageDisplay);

            // CPU USAGE DISPLAY
            wchar_t cpuUsageDisplay[256] = { 0 };
